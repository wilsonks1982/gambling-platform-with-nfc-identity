 ...
 * |  28/07/2025   | Wilson Sam   | Use UserService |  Fetch and update user info and wallet from UserService
 * |  28/07/2025   | Wilson Sam   |   Updated Logic |  Support for full MultiHandDrawResultResponse fields
 * |  28/07/2025   | Wilson Sam   |  Multi-deck     |  Use separate deck for each hand in multi-hand mode
 * |  28/07/2025   | Wilson Sam   |  Add Logging    |  Added slf4j logging, especially for hold/draw debugging
 * |  28/07/2025   | Wilson Sam   | Primary Copy    |  On draw, copy held cards from primary hand to all hands
 * |  29/07/2025   | Wilson Sam   |  Deep Copy Decks|  Ensure decks are deep copied to avoid shared mutable state
 * |  29/07/2025   | Wilson Sam   |  Audit Logging  |  Added GameAuditLogRepository for audit logs
 * |  29/07/2025   | Wilson Sam   |  Scheduled Task |  Added cleanup task to remove old sessions after 30 mins
 * |  29/07/2025   | Wilson Sam   |  Error Handling |  Improved error handling and response messages
 * |  29/07/2025   | Wilson Sam   |  Date Formatting|  Use consistent date formatting for gameStart/gameEnd 
 * |  29/07/2025   | Wilson Sam   |  Session Expiry |  Added session expiry logic to cleanup old games 
 * |  29/07/2025   | Wilson Sam   |  Game Results   |  Added results endpoints for querying game history
...
import com.wildace.poker.jacksorbetter.repository.DeucesWildGameAuditLogRepository;
 ...
 */
@Service
public class DeucesWildGameService {

    private static final Logger log = LoggerFactory.getLogger(DeucesWildGameService.class);

    private final UserService userService;
    private final DeucesWildGameAuditLogRepository auditLogRepository;
    private final DeucesWildPayTableService deucesWildPayTableService;


    @Autowired
    public DeucesWildGameService(UserService userService, DeucesWildGameAuditLogRepository auditLogRepository, DeucesWildPayTableService deucesWildPayTableService) {
        this.userService = userService;
        this.auditLogRepository = auditLogRepository;
        this.deucesWildPayTableService = deucesWildPayTableService;
    }
    ...
    public DealResponse deal(DealRequest dealRequest) {
        ...
        DeucesWildGameAuditLog dealAudit = new DeucesWildGameAuditLog();
    ...
    public MultiHandDrawResultResponse draw(String sessionId, List<Integer> holds) {
        ...
        for (int handIndex = 0; handIndex < state.numberOfHands; handIndex++) {
            ...
            String result = DeucesWildPokerHandEvaluator.evaluateHand(newHand);
            int payout = deucesWildPayTableService.getPayoutForHand(result, state.coin);
        ...
        DeucesWildGameAuditLog drawAudit = new DeucesWildGameAuditLog();
